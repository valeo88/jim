...
public class RebalanceServiceImpl implements RebalanceService {
    ...
    public PortfolioRebalancePropositionDto rebalance(@NotNull Portfolio portfolio) {
        ...
        if (!targetPercentDistribution.isEmpty()) {
            ...
            Map<InstrumentCategory, BigDecimal> totalPriceByCategoryActual = new HashMap<>();
            for (var position: portfolio.getPositions()) {
                ...
                totalPriceByCategoryActual.compute(position.getInstrument().getCategory(), (k,v) -> {
                    if (isNull(v)) {
            ...
            final var totalInstrumentsPrice = totalPriceByCategoryActual.values().stream()
                    .reduce(BigDecimal::add).orElse(BigDecimal.ZERO);
            Map<InstrumentCategory, BigDecimal> totalPriceByCategoryTarget = targetPercentDistribution.entrySet()
                    .stream().map(entry -> {
                        entry.setValue(entry.getValue().multiply(totalInstrumentsPrice)
